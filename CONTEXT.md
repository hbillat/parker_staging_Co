...
- **Lead Scraper Feature** (NEW): Generate business leads from Google searches
...
│   └── leads-table.tsx           # Leads table display
├── lib/
│   ├── supabase/
...
#### Duplicate Detection
- Checks `business_name` + `address` combination
- Before inserting, queries existing leads for the project
- If duplicate found, skips insertion and increments counter
- Prevents same business appearing multiple times
...
- **LeadsTable**: Table display of all collected leads with sortable columns
...
5. **No Export Feature**: Can't export leads to CSV (yet)
...
- **supabase_migration.sql**: Database schema for lead scraper
